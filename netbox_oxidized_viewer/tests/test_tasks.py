...
class TestUpdateConfigSnapshots(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.spine = Device.objects.create(name='spine1', site=site, device_type=device_type, role=role)
        cls.leaf = Device.objects.create(name='leaf1', site=site, device_type=device_type, role=role)
        # A device with no matching file in the repo — must be skipped silently.
        cls.orphan = Device.objects.create(name='no-backup', site=site, device_type=device_type, role=role)
    ...
    def test_creates_snapshots_for_matched_devices(self):
        ...
        self.assertEqual(set(snaps.values_list('device__name', flat=True)), {'spine1', 'leaf1'})
    ...
    def test_backfills_metadata_for_pre_0006_rows(self):
        ...
        update_config_snapshots(source_pk=self.source.pk)
        ConfigSnapshot.objects.filter(device=self.spine).update(commit_timestamp=None, commit_subject='')
        update_config_snapshots(source_pk=self.source.pk)
        snap = ConfigSnapshot.objects.get(device=self.spine)
    ...
    def test_search_vector_populated(self):
        ...
        self.assertEqual(set(hits.values_list('device__name', flat=True)), {'spine1'})
