...
from netbox_oxidized_viewer.utils import resolve_device_field, get_source
...
class TestResolveDeviceField(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.device = Device.objects.create(
            ...
            serial='SN-123', asset_tag='ASSET-9',
        )
    ...
    def test_resolve_custom_field(self):
        self.assertEqual(
            resolve_device_field(self.device, 'cf_oxidized_name'), 'spine1-cfg'
        )
    ...
    def test_resolve_ip_proxy_object(self):
        ...
        self.device.fake_ip = SimpleNamespace(
            address=SimpleNamespace(ip='10.0.0.1')
        )
...
class TestGetSource(TestCase):
    ...
    def test_returns_source_when_present(self):
        source = OxidizedSource.objects.create(
            name='Lab', git_repo_path='/tmp/repo'
        )
